...
class FilePanel(tk.Frame):
    ...
    def _build_pathbar(self, t):
        ...
        self._path_entry.bind("<Return>", lambda _e: self.navigate(self._path_var.get()))
    ...
    def _build_tree(self, t):
        ...
        self._tree.tag_configure(
            "folder", foreground=t["tag_folder"], font=("Segoe UI", 9, "bold")
        )
    ...
    def _build_context_menu(self, t):
        ...
        self._menu.add_separator()
        self._menu.add_command(label="Renombrar", command=self._rename_selected)
        self._menu.add_command(label="Eliminar", command=lambda: self._delete_selected(False))
        self._menu.add_command(
            label="Eliminar permanente", command=lambda: self._delete_selected(True)
        )
        self._menu.add_separator()
        ...
        self._menu.add_command(label="Propiedades", command=self._show_props)
    ...
    def navigate(self, path: str):
        ...
        if not self._is_accessible_directory(path):
            self._status.set(f"Ruta no valida o inaccesible: {path}")
            return
            ...
            for entry in folders:
                self._tree.insert(
                    ...
                    values=(
                        ...
                        "Carpeta",
                        "-",
                        self._fmt_date(entry.stat().st_mtime),
                    ),
                    tags=("folder",),
                )
            for entry in files:
    ...
    def _push_history(self, path: str):
        if self._hist_pos >= 0 and self._history[self._hist_pos] == path:
            return
    ...
    def load_search_results(self, results):
        ...
        for path, name, ftype, size, mtime in results:
            self._tree.insert(
                ...
                values=(
                    f"  {name}",
                    ftype if ftype != "folder" else "Carpeta",
                    self._fmt_size(size) if ftype != "folder" else "-",
                    self._fmt_date(mtime),
                ),
                tags=("folder" if ftype == "folder" else "file",),
            )
    ...
    def _copy_selected(self):
        selection = self.get_selected_paths()
        self.file_ctrl.copy_to_clipboard(selection)
        self._status.set(f"  {len(selection)} elemento(s) copiado(s)")

    def _cut_selected(self):
        selection = self.get_selected_paths()
        self.file_ctrl.cut_to_clipboard(selection)
        self._status.set(f"  {len(selection)} elemento(s) cortado(s)")
    ...
    def _delete_selected(self, permanent=False):
        ...
        if not selection:
            return
        names = [os.path.basename(path) for path in selection]
        if confirm_delete(self, names, permanent):
            self.file_ctrl.delete(selection, permanent)
            self.refresh()
    ...
    def _show_props(self):
        ...
        try:
            ...
            info = {
                ...
                "Tamano": self._fmt_size(stat.st_size),
